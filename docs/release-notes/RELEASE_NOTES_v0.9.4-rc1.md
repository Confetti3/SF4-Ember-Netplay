...
After a session, send `sf4e.log` (see [saving logs](../guides/SAVING_LOGS.md)) with a note on how the match felt, your input delay, and whether the route was direct or relayed. The lines starting `Pacing [` and `FreezeCandidate` carry the numbers we need.
